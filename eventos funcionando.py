...
class Calend(ttk.Frame):
    def __init__(self, master= None):
        ...
        self.create_button.config(cursor= 'hand2',fg='black', bg='purple',font=('Arial',12,'bold'), activebackground='cyan')
        ...
        self.quit_button.config(cursor= 'hand2',fg='black', bg='yellow',font=('Arial',12,'bold'), activebackground='cyan')
        ...
        self.eti_sem=ttk.Label(self.frame2, text='')
        self.eti_sem.pack(padx=20, pady=20)

        self.tab_sem= ttk.Treeview(self.frame2, columns=('hora','fecha','evento', 'importancia', 'etiqueta'), show= 'headings', height= '20')
        #self.tabla.grid(row=2, column=1, pady=10, padx=10)
        self.tab_sem.heading('hora', tex= 'Horario',anchor=CENTER)
        self.tab_sem.heading('fecha', tex= 'Fecha',anchor=CENTER)
        self.tab_sem.heading('evento', tex= 'Evento',anchor=CENTER)
        self.tab_sem.heading('importancia', tex= 'Importancia',anchor=CENTER)
        self.tab_sem.heading('etiqueta', tex= 'Etiqueta',anchor=CENTER)
        
        self.tab_sem.column('hora', width=80,anchor=CENTER)
        self.tab_sem.column('fecha', width=150,anchor=CENTER)
        self.tab_sem.column('evento', width=180,anchor=CENTER)
        self.tab_sem.column('importancia', width=180,anchor=CENTER)
        self.tab_sem.column('etiqueta', width=180,anchor=CENTER)

        self.tab_sem.pack(padx=10, pady=20)
    ...
    def seleccion(self, event):
        ...
        ini=datetime.strptime(self.date_entry, '%m/%d/%y') -timedelta(days=datetime.strptime(self.date_entry, '%m/%d/%y').weekday()+1)
        f_ini=datetime.strftime(ini,'%d/%m/%y')
        fin=ini+timedelta(days=6)
        f_fin=datetime.strftime(fin, '%d/%m/%y')
        self.eti_sem.configure(text='Semana del domingo '+f_ini+' al sabado '+f_fin)
        
        self.mostrar_datos_semana(self.tab_sem,ini,fin)

    def mostrar_datos_semana(self, tree, ini, fin):
        datos = self.leer_archivo_json()
        
        datos_filtrados = [dato for dato in datos if ini<=datetime.strptime(dato['fecha'],'%m/%d/%y')<=fin ]
        #ordeno la lista
        datos_ordenados = sorted(datos_filtrados, key=lambda x: datetime.strptime(x['fecha'] + ' ' + x['hora'], '%m/%d/%y %H:%M'))
        ...
        for dato in datos_ordenados:
            tree.insert('', 'end', text=dato['hora'], 
                        values=(dato['hora'],dato['fecha'],dato['titulo'], dato['importancia'], dato['etiquetas']))                       
    
#ventan del evento semanal
    def create_ventana(self):
        self.event_window = Toplevel(self.frame1)
        
        self.event_window.geometry("850x600")
        self.event_window.title("Eventos del Dia: " + self.date_entry)
        self.event_window.config( bg= "SkyBlue1",  border=2, relief= "groove")
         # Crear un Entry para ingresar el nuevo evento
        self.event_entry = Entry(self.event_window, bg= "lemon chiffon",  border=2, relief= "groove")
        self.fondo=ImageTk.PhotoImage(Image.open("river.png"))
        self.i_fondo= Label(self.event_window,image=self.fondo)
        self.i_fondo.grid(row=6, column=1, pady=10, padx=10,columnspan=2)

        #tabla de eventos del día
        
        self.tabla= ttk.Treeview(self.event_window, columns=('hora','evento', 'importancia', 'etiqueta'), show= 'headings', height= '15')
        
        self.tabla.heading('hora', tex= 'Horario',anchor=CENTER)
        self.tabla.heading('evento', tex= 'Evento',anchor=CENTER)
        self.tabla.heading('importancia', tex= 'Importancia',anchor=CENTER)
        self.tabla.heading('etiqueta', tex= 'Etiqueta',anchor=CENTER)
        self.tabla.column('hora', width=80,anchor=CENTER)
        self.tabla.column('evento', width=150,anchor=CENTER)
        self.tabla.column('importancia', width=150,anchor=CENTER)
        self.tabla.column('etiqueta', width=150,anchor=CENTER)
        ...
        self.tabla.grid(row=1, column=1, pady=10, padx=10, rowspan= 5)
        ...
        self.save_button.grid(row=2, column=2, pady=10, padx=10)
        self.save_button.config(cursor= 'hand2',fg='black', bg='blue',font=('Arial',12,'bold'), activebackground='green2')

        # Crear un mensaje indicando como editar el evento
        
        self.edit_button= tk.Label(self.event_window, text= "Haga Doble Click \nsobre el evento para Editar", fg='red', bg='cyan')
        self.edit_button.grid(row=3, column= 2, pady=10, padx=10)
        self.edit_button.config(cursor= 'question_arrow',width=30)
        
        self.delete_button = tk.Button(self.event_window, text="Eliminar Evento", state="disabled", command=self.eliminar_evento)
        self.delete_button.grid(row=4, column=2, pady=10, padx=10)
        self.delete_button.config(cursor= 'hand2',fg='black', bg='tomato',font=('Arial', 12,'bold'), activebackground='green2')

        self.cerrar_button = tk.Button(self.event_window, text="Cerrar", command=self.event_window.destroy)
        self.cerrar_button.grid(row=5, column=2, pady=10, padx=10)
        self.cerrar_button.config(cursor= 'hand2',fg='black', bg='pink',font=('Arial',12,'bold'), activebackground='green2')
    ...
    def vent_create__event(self):
        ...
        self.evento_window.config( bg= "SkyBlue3",  border=2, relief= "groove")
        ...
        self.fecha_label = tk.Label(self.evento_window, text="Fecha:",anchor=CENTER)
        self.fech_entry = tk.Entry(self.evento_window)
        self.fech_entry.insert(0,self.date_entry)
        self.fech_entry.config(state= 'disable')

        self.date_label = tk.Label(self.evento_window, text="Hora:",anchor=CENTER)
        ...
        self.duration_label = tk.Label(self.evento_window, text="Duración:",anchor=CENTER)
        self.duration_entry = tk.Entry(self.evento_window)
        self.duration_entry.insert(0, "1")

        self.description_label = tk.Label(self.evento_window, text="Descripción:",anchor=CENTER)
        self.description_entry = tk.Text(self.evento_window, width=20, height=4)

        self.importance_label = tk.Label(self.evento_window, text="Importancia:",anchor=CENTER)
        ...
        self.reminder_label = tk.Label(self.evento_window, text="Recordatorio:",anchor=CENTER)
        self.reminder_entry = tk.Entry(self.evento_window)

        self.tags_label = tk.Label(self.evento_window, text="Etiquetas:",anchor=CENTER)
    ...
    def doubleClickTabla(self,event):
        ...
        self.evento_window.geometry("300x400")
        self.evento_window.config( bg= "linen",  border=2, relief= "groove")
        ...
        self.di_create_button.config( cursor= 'hand2',fg='black', bg='green',font=('Arial',12,'bold'), activebackground='yellow')
        
        self.di_quit_button = tk.Button(self.evento_window, text="Cancelar", 
                                     command=self.evento_window.destroy)
        self.di_quit_button.config(cursor= 'hand2',fg='black', bg='red',font=('Arial',12,'bold'), activebackground='green2')
